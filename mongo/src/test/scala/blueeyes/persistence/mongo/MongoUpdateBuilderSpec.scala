...
class MongoUpdateBuilderSpec extends Specification{
  "builds $inc operation" in {
    ("n" inc (1) toJValue) mustEqual (JObject(JField("$inc", JObject(JField("n", JNum(1)) :: Nil)) :: Nil))
  }
  "builds $set operation" in {
    ("n" set (1) toJValue) mustEqual (JObject(JField("$set", JObject(JField("n", JNum(1)) :: Nil)) :: Nil))
  }
  "builds $unset operation" in {
    (("n" unset) toJValue) mustEqual (JObject(JField("$unset", JObject(JField("n", JNum(1)) :: Nil)) :: Nil))
  }
  "builds popLast operation" in {
    (("n" popLast) toJValue) mustEqual (JObject(JField("$pop", JObject(JField("n", JNum(1)) :: Nil)) :: Nil))
  }
  "builds popFirst operation" in {
    (("n" popFirst) toJValue) mustEqual (JObject(JField("$pop", JObject(JField("n", JNum(-1)) :: Nil)) :: Nil))
  }

  "builds $push operation" in {
    ("n" push (1) toJValue) mustEqual (JObject(JField("$push", JObject(JField("n", JNum(1)) :: Nil)) :: Nil))
  }
  "builds $pull operation with default operator" in {
    (("n" pull(JPath("") === 1)).toJValue) mustEqual (JObject(JField("$pull", JObject(JField("n", JNum(1)) :: Nil)) :: Nil))
  }
  "builds $pull operation with custom operator" in {
    (("n" pull(JPath("") > 1)).toJValue) mustEqual (JObject(JField("$pull", JObject(JField("n", JObject(JField("$gt", JNum(1)) :: Nil)) :: Nil)) :: Nil))
  }
