...
class MockMongo extends Mongo with AkkaDefaults {
  ...
  lazy val close: Future[Unit] = Future(())
}

private[mongo] class MockDatabase(val mongo: Mongo)(implicit messageDispatcher: MessageDispatcher) extends Database {
  ...
  def applyQuery[T <: MongoQuery](query: T, isVerified: Boolean)(implicit m: Manifest[T#QueryResult], queryTimeout: Timeout): Future[T#QueryResult] = Future(query(query.collection, isVerified))

  def collections = databaseCollections.entrySet.map(entry => MongoCollectionHolder(entry.getValue, entry.getKey, this)).toSet

  lazy val disconnect = Future(())
}
...
private[mongo] class MockDatabaseCollection(val name: String, val database: MockDatabase) extends DatabaseCollection with JObjectFields with MockIndex with ReadWriteLock with MongoFilters {
  ...
  def count(filter: Option[MongoFilter]) = safeProcess(filter) { _.size }

  def distinct(selection: JPath, filter: Option[MongoFilter]) =
    safeProcess(filter) { _.flatMap(selectByPath(selection, _, Some(_), (p, v) => v)).distinct }
