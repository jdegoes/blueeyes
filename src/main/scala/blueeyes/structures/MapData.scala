 ...
 *  as well.  The MAX_HISTORY variable determines how large the history vector
 *  can get before it resets and a new MapData object is created
 */
...
case class MapData[K, V](
  ...
  def rollBackToVersion(newVersion: Int): MapData[K, V] = {
    if (version >= MapData.MAX_HISTORY) {
      ...
      // if version is not up to date, return a copy with the "newer" operations undone
