...
case class MongoQueryBuilder(jpath: JPath) {
  ...
  def > (value: Long): MongoFieldQuery = MongoFieldQuery(jpath, $gt, value)

  def >= (value: Long): MongoFieldQuery = MongoFieldQuery(jpath, $gte, value)

  def < (value: Long): MongoFieldQuery = MongoFieldQuery(jpath, $lt, value)
