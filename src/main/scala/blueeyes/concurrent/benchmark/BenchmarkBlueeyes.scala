...
object BenchmarkBlueeyes extends BenchmarkScenario[Actor[String, Unit]]{
  ...
  private val strategy = new ActorExecutionStrategyFixedPool {
    val actorExecutionStrategyThreadPoolSize = 50
  }
  ...
  private def actor(index: Int, actors: Array[Actor[String, Unit]], cdl: CountDownLatch) = Actor[String, Unit, Tuple3[Int, Array[Actor[String, Unit]], CountDownLatch]]((index, actors, cdl)){ state => {
    case x: String =>
