 ...
 * <p>
 * val street = (!json \ "address" \ "street" asUnsafe JString).values
 * <p>
...
object Validation {
  ...
  case class JPathValue(path: String, value: JValue) {
     ...
     * informative errors.
     */
    def !! (i: Int) = JPathValue(path + "[" + i + "]", (this asUnsafe JArray).elements(i))
  ...
  def arrayFieldMap[A](name: String, json: JPathValue, f: JPathValue => A): List[A] = {
    val field = json \ name 
    val array = field asUnsafe JArray
   ...
   * the string.
   */
  def stringField(name: String, json: JPathValue) = (json \ name asUnsafe JString).value
   ...
   * integer as a BigInt.
   */
  def integerField(name: String, json: JPathValue) = (json \ name asUnsafe JInt).value
  
  /**
   * Utility function to extract the specified field as a JDouble, then return 
   * the double as a Double.
   */
  def doubleField(name: String, json: JPathValue) = (json \ name asUnsafe JDouble).value
