...
object CacheDirectives {
  ...
  def noFieldDirectives: Array[CacheDirective] = (noFieldRegex.findAllIn(inString)).toArray.map(_.trim.split("=")).map( _ match {
      ...
      case Array("max-age", delta) => `max-age`(Some(delta.toLong))
      case Array("max-stale", delta) => `max-stale`(Some(delta.toLong))
      case Array("min-fresh", delta) => `min-fresh`(Some(delta.toLong))
      ...
      case Array("s-maxage", delta) => `s-maxage`(Some(delta.toLong))
      case default => NullDirective("null")
    })
    return noFieldDirectives ++ fieldDirectives
  }
  ...
  case class `max-age`(inDelta: Option[Long]) extends RequestDirective {
    override def delta = inDelta
  }
  case class `max-stale`(inDelta: Option[Long]) extends RequestDirective {
    override def delta = inDelta
  }
  case class `min-fresh`(inDelta: Option[Long]) extends RequestDirective {
    override def delta = inDelta
  }
  ...
  //case class `max-age`(delta: Option[Long])  extends ResponseDirective  with NoFieldName 
  case class `s-maxage`(inDelta: Option[Long])  extends ResponseDirective { 
    override def delta = inDelta
  }
