...
class StrategyThreadedNSpec extends Specification{
  ...
  "StrategyThreadedN: handle musltiple requests from multiple threads for multiple functions" in{
    ...
    entries foreach { f =>
      executor execute(new Runnable{
        def run = {
          ...
          strategy.actorExecutionStrategy.queues.size must beLessThan (functions.size + 1)
          strategy.actorExecutionStrategy.unassignedQueues.size must beLessThan (functions.size + 1)
        }
      })
    }
    ...
    strategy.actorExecutionStrategy.queues.size must be (0)
    strategy.actorExecutionStrategy.unassignedQueues.size must be (0)
  }
