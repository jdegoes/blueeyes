...
class WithinFilterEvaluatorSpec extends Specification {
  ...
  "WithinFilterEvaluator.centerSphere" should {
    "return true when object has int array geo fields and point is in the circle" in {
      WithinFilterEvaluator(JArray(JInt(15) :: JInt(15) :: Nil), withinCenterSphere) must be (true)
    }
    "return false when object has int array geo fields and point is not in the circle" in {
      WithinFilterEvaluator(JArray(JInt(45) :: JInt(15) :: Nil), withinCenterSphere) must be (false)
    }
    "return true when object has double JObject geo fields and point is in the circle" in {
      WithinFilterEvaluator(JObject(JField("x", JDouble(12.0)) :: JField("y",  JDouble(11.0)) :: Nil), withinCenterSphere) must be (true)
    }
    "return false when object has double JObject geo fields and point is not in the circle" in {
      WithinFilterEvaluator(JObject(JField("x", JDouble(42.0)) :: JField("y",  JDouble(11.0)) :: Nil), withinCenterSphere) must be (false)
    }
    "throws MongoExceptions if Radians is too big" in {
      WithinFilterEvaluator(JObject(JField("x", JDouble(42.0)) :: JField("y",  JDouble(11.0)) :: Nil), JObject(JField("$within", JObject(JField("$centerSphere",  JArray(JArray(JDouble(10.0) :: JDouble(10.0) :: Nil) :: JDouble(0.5) :: Nil)) :: Nil)) :: Nil)) must be (false)
    }
  }
