...
class ActorMSpec extends Specification with ScalaCheck {
  ...
  val doubleEveryOther: ActorM[Option, Int, Int] = {
    def d0(double: Boolean): ActorM[Option, Int, Int] = receive { a: Int =>
      reply(if (double) a * 2 else a, d0(!double))
    }
  ...
  "Actor !" should {
    "return result" in {
      (double ! 2)._1 mustEqual Some(4)
    }
  }

  "Actor !!" should {
    "return sequence of results for stateless actor in Option monad" in {
      (double !! (2, 4, 6))._1.toList mustEqual (Some(4) :: Some(8) :: Some(12) :: Nil)
    }

    "return sequence of results for stateful actor in Option monad" in {
      (doubleEveryOther !! (2, 4, 6))._1.toList mustEqual (Some(4) :: Some(4) :: Some(12) :: Nil)
    }
  }

  "Actor >>>" should {
    "compose actors in the option monad" in {
      ((double >>> halve) !! (1, 2, 3, 4))._1.toList mustEqual (1 :: 2 :: 3 :: 4 :: Nil).map(Some.apply _)
    }
  }
}